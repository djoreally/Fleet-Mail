...
function InboxPage({ go }: { go: (page: PageId) => void }) {
  const [filter, setFilter] = useState<'all' | 'unread'>('all');
  const emails = [
    { id: 'msg-1', sender: 'Redwood Logistics', email: 'sarah@redwoodlogistics.com', subject: 'Urgent: Brake check on TRK-104', snippet: 'Noticeable pad wear reported on driver shift change. Need inspection WO created before 2:30 PM.', date: '10:14 AM', unread: true, wo: 'WO-2481' },
    { id: 'msg-2', sender: 'Atlas Building Supply', email: 'mike@atlasbuild.com', subject: 'Re: Coolant leak on TRK-118', snippet: 'Authorized to proceed with cooling pressure test at Tulsa depot.', date: 'Yesterday', unread: true, wo: 'WO-2458' },
    { id: 'msg-3', sender: 'Northstar Foods', email: 'dispatch@northstarfoods.com', subject: 'Scheduled PM for VAN-028', snippet: 'Confirming mobile technician arrival window tomorrow at Austin location.', date: 'Aug 29', unread: false, wo: 'WO-2477' },
  ];
  return <PageFrame>
    <SectionHeading eyebrow="AgentMail Inbox · moms@agentmail.to" title="Fleet Inbox" description="Incoming customer service requests, authorizations, and driver reports automatically triaged for Fleet OS." action={<Button icon={Plus} onClick={() => go('work-orders')}>Create work order</Button>} />
    <div className="mt-6 flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
      <div className="flex gap-1 rounded-lg border border-slate-200 bg-slate-50/70 p-1">
        <button onClick={() => setFilter('all')} className={cn('rounded-md px-3 py-1.5 text-xs font-semibold', filter === 'all' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500')}>All mail</button>
        <button onClick={() => setFilter('unread')} className={cn('rounded-md px-3 py-1.5 text-xs font-semibold', filter === 'unread' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500')}>Unread (2)</button>
      </div>
      <span className="text-xs font-medium text-slate-400">3 messages linked to active work orders</span>
    </div>
    <section className="mt-4 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="divide-y divide-slate-100">
        {emails.filter(e => filter === 'all' || e.unread).map(email => (
          <div key={email.id} className={cn('flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between transition hover:bg-slate-50', email.unread && 'bg-blue-50/30')}>
            <div className="flex items-start gap-3 min-w-0 flex-1">
              <span className={cn('mt-1.5 h-2 w-2 shrink-0 rounded-full', email.unread ? 'bg-blue-600' : 'bg-transparent')} />
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-slate-900">{email.sender}</span>
                  <span className="text-xs text-slate-400">&lt;{email.email}&gt;</span>
                  <span className="ml-auto text-xs text-slate-400 sm:ml-0">{email.date}</span>
                </div>
                <p className="mt-0.5 text-sm font-semibold text-slate-800">{email.subject}</p>
                <p className="mt-1 text-xs text-slate-500">{email.snippet}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <span className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-bold text-slate-600">{email.wo}</span>
              <Button variant="secondary" onClick={() => go(email.wo === 'WO-2481' ? 'inspection' : 'work-orders')} icon={ArrowRight}>View WO</Button>
            </div>
          </div>
        ))}
      </div>
