...
export const FleetModuleView:React.FC<FleetModuleViewProps>=({module})=>{const screens:Record<FleetModuleId,React.ReactNode>={vehicles:<VehiclesLive/>,'work-orders':<WorkOrdersWorkspace/>,maintenance:<MaintenanceWorkspace/>,schedule:<ScheduleDispatchView mode="schedule"/>,dispatch:<ScheduleDispatchView mode="dispatch"/>,parts:<PartsWorkspace/>,prospects:<ProspectCommandCenter/>,customers:<CustomersWorkspace/>,financials:<FinancialsLiveView/>,documents:<DocumentsLiveView/>};return <div className="flex min-h-0 flex-1 flex-col">{screens[module]}<div className="mx-5 mb-5 mt-1 flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm lg:mx-8 lg:mb-8"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-slate-400">Fleet lifecycle</p><p className="mt-1 text-xs font-semibold text-slate-700">Prospect → Win → Fleet Account → Vehicle → Work Order → Service → Invoice → Retain</p></div><Sparkles className="h-5 w-5 text-blue-500"/></div></div>};
