...
export function SignInPage({ onNavigate, onAuthenticated }: SignInPageProps) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      onAuthenticated?.();
      onNavigate('/app');
    } catch (reason) { setError(getAuthError(reason, 'We could not sign you in. Check your email and password.')); }
    finally { setLoading(false); }
  }

  return (
    <AuthShell eyebrow={invited ? 'Team invitation' : 'Welcome back'} title={invited ? 'Sign in to join your team' : 'Sign in to your operation'} description={invited ? 'Use the same work email that received the invitation.' : 'Continue to your fleet command center.'} onNavigate={onNavigate}>
      <form onSubmit={submit} className="space-y-5">
        ...
        <AuthSubmit loading={loading}>{invited ? 'Sign in and join' : 'Sign in'} <ArrowRight className="ml-2 h-4 w-4" /></AuthSubmit>
      </form>
