...
export const operationsApi={
  ...
  customerOverview:async(id:string)=>{const response=await fleetFetch(`/api/fleet-service/account-overview?id=${encodeURIComponent(id)}`);const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||`Request failed (${response.status})`);return data as {account:FleetAccount360}},
