...
const CLOSED_STAGES=new Set(['won','lost','converted']);
...
export class ProspectingService {
 ...
 async attentionQueue(organizationId:string){
  const db=database();const rows=await db.select().from(prospects).where(eq(prospects.organizationId,organizationId)).orderBy(desc(prospects.updatedAt)).limit(500);const active=rows.filter(p=>!CLOSED_STAGES.has(p.stage));const ids=active.map(p=>p.id);const activityRows=ids.length?await db.select().from(prospectActivities).where(eq(prospectActivities.organizationId,organizationId)).orderBy(desc(prospectActivities.occurredAt)).limit(3000):[];const latest=new Map<string,(typeof activityRows)[number]>();for(const a of activityRows)if(!latest.has(a.prospectId))latest.set(a.prospectId,a);const now=Date.now();const day=86400000;
  const queue=active.map(p=>{const a=latest.get(p.id);const due=p.nextFollowUpAt?new Date(p.nextFollowUpAt).getTime():null;const last=a?new Date(a.occurredAt).getTime():new Date(p.updatedAt).getTime();const ageDays=Math.max(0,Math.floor((now-last)/day));const inbound=Boolean(a&&a.direction==='inbound');let priority=0,reason='',nextAction='';if(inbound){priority=100;reason='New inbound reply needs a response';nextAction='Review reply and respond';}else if(due!==null&&due<=now){priority=90+Math.min(9,Math.floor((now-due)/day));reason='Follow-up is overdue';nextAction='Follow up now';}else if(['engaged','meeting','proposal'].includes(p.stage)&&ageDays>=2){priority=75+Math.min(10,ageDays);reason=`Active opportunity has been quiet for ${ageDays} days`;nextAction=p.stage==='proposal'?'Check proposal status':'Re-engage the prospect';}else if(p.stage==='outreach'&&ageDays>=3){priority=65+Math.min(10,ageDays);reason=`No reply after ${ageDays} days`;nextAction='Send a concise follow-up';}else if(p.stage==='qualified'&&!p.lastContactedAt){priority=55;reason='Qualified prospect has not been contacted';nextAction='Start outreach';}else if(due!==null&&due<=now+day){priority=45;reason='Follow-up is due within 24 hours';nextAction='Prepare follow-up';}return {...p,attentionPriority:priority,attentionReason:reason,suggestedNextAction:nextAction,lastActivityAt:a?.occurredAt??null,lastActivityKind:a?.kind??null,lastActivityDirection:a?.direction??null,ageDays};}).filter(p=>p.attentionPriority>0).sort((a,b)=>b.attentionPriority-a.attentionPriority||Number(b.opportunityValue||0)-Number(a.opportunityValue||0));
  return {items:queue.slice(0,100),summary:{needsAttention:queue.length,overdue:queue.filter(x=>x.attentionReason==='Follow-up is overdue').length,repliesWaiting:queue.filter(x=>x.lastActivityDirection==='inbound').length,dueSoon:queue.filter(x=>x.attentionReason==='Follow-up is due within 24 hours').length}};
 }
