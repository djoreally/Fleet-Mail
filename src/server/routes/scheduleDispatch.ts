...
const snake=(row:any)=>Object.fromEntries(Object.entries(row).map(([key,value])=>[key.replace(/[A-Z]/g,m=>`_${m.toLowerCase()}`),value]));
...
scheduleDispatchRouter.get('/appointments', run(async req => { const c=await context(req); return (await db().select().from(appointments).where(eq(appointments.organizationId,c.organizationId)).orderBy(asc(appointments.startsAt))).map(snake); }));
...
scheduleDispatchRouter.get('/dispatch', run(async req => { const c=await context(req); return (await db().select().from(dispatchAssignments).where(eq(dispatchAssignments.organizationId,c.organizationId))).map(snake); }));
