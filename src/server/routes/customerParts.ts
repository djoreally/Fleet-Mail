...
customerPartsRouter.delete('/customers/:id',async(req,res)=>{try{const org=await requireFleetOrganization(req);await requireFleetPermission(req,'fleet_accounts.manage');return res.json({deleted:await operationsDataService.deleteCustomer(org,req.params.id)})}catch(e){return fail(res,e)}});
...
customerPartsRouter.post('/parts',async(req,res)=>{try{const org=await requireFleetOrganization(req);await requireFleetPermission(req,'work_orders.manage');return res.status(201).json({part:await operationsDataService.createPart(org,req.body??{})})}catch(e){return fail(res,e)}});
customerPartsRouter.put('/parts/:id',async(req,res)=>{try{const org=await requireFleetOrganization(req);await requireFleetPermission(req,'work_orders.manage');return res.json({part:await operationsDataService.updatePart(org,req.params.id,req.body??{})})}catch(e){return fail(res,e)}});
customerPartsRouter.put('/parts/:id/inventory',async(req,res)=>{try{const org=await requireFleetOrganization(req);await requireFleetPermission(req,'work_orders.manage');return res.json({inventory:await operationsDataService.adjustInventory(org,req.params.id,req.body?.quantity,req.body?.reorderPoint)})}catch(e){return fail(res,e)}});
customerPartsRouter.delete('/parts/:id',async(req,res)=>{try{const org=await requireFleetOrganization(req);await requireFleetPermission(req,'work_orders.manage');return res.json(await deleteInventoryPart(org,req.params.id))}catch(e){return fail(res,e)}});
