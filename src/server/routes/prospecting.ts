...
prospectingRouter.post('/prospects/:id/convert',async(req,res)=>{try{const org=await requireFleetOrganization(req);await prospectingService.get(org,req.params.id);return res.status(202).json({action:createAgentActionProposal('fleet.prospect.convert',{prospectId:req.params.id})});}catch(e){return fail(res,e)}});
