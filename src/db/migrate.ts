...
/**
 * Applies checked-in forward SQL exactly once per database.
 * Runtime Vercel requests are denied by default so /api/neon/migrate cannot act
 * as an unauthenticated production control plane. Deployment/CLI callers must
 * explicitly opt in through allowRuntime.
 */
export async function runDrizzleMigration(
  customDatabaseUrl?: string,
  options: { allowRuntime?: boolean } = {},
): Promise<MigrationReport> {
  if (process.env.VERCEL && options.allowRuntime !== true) {
    throw new Error('Runtime database migrations are disabled on Vercel');
  }
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  runDrizzleMigration(undefined, { allowRuntime: true })
