...
const ATLASCLOUD_MODEL = 'dots-studio/dots-3-note-prev-free';
const AGENTMAIL_BASE_URL = 'https://api.agentmail.to/v0';
const DEFAULT_INBOX = process.env.DEFAULT_INBOX || 'moms@agentmail.to';

// Helper to initialize the AgentMail SDK client
let agentMailClientInstance: AgentMailClient | null = null;
function getAgentMailClient(): AgentMailClient | null {
...
// Helper to call AtlasCloud or fallback to Gemini
async function callAICompletion(messages: Array<{ role: string; content: string }>, systemPrompt?: string) {
  ...
  if (atlasKey && atlasKey !== 'your-atlascloud-api-key' && atlasKey.trim() !== '') {
    try {
      const response = await fetch(`${ATLASCLOUD_BASE_URL}/chat/completions`, {
        method: 'POST',
        headers: {
      ...
      console.warn('AtlasCloud call failed, attempting fallback if available:', err.message);
      if (!geminiKey) throw err;
    }
  }

  // Fallback to Gemini if AtlasCloud key isn't provided or failed
  if (geminiKey) {
...
const NEON_DATA_API_URL = process.env.NEON_DATA_API_URL || process.env.VITE_NEON_DATA_API_URL || 'https://ep-sparkling-pine-afqf3sia.apirest.c-2.us-west-2.aws.neon.tech/neondb/rest/v1';
const NEON_AUTH_URL = process.env.NEON_AUTH_URL || process.env.VITE_NEON_AUTH_URL || 'https://ep-sparkling-pine-afqf3sia.neonauth.c-2.us-west-2.aws.neon.tech/neondb/auth';
...
app.post('/api/agentmail/simulate-incoming', async (req, res) => {
  try {
...
app.post('/api/contacts/extract-from-inbox', (req, res) => {
...
app.post('/api/neon/migrate', async (req, res) => {
  try {
    const { runDrizzleMigration } = await import('./src/db/migrate.js').catch(() => import('./src/db/migrate'));
...
app.post('/api/drizzle/migrate', async (req, res) => {
  try {
    const { runDrizzleMigration } = await import('./src/db/migrate.js').catch(() => import('./src/db/migrate'));
    const report = await runDrizzleMigration();
    return res.json(report);
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`ChatMail AI Server running on http://0.0.0.0:${PORT}`);
  });
}
