...
describe('AgentMail prospect webhook contract',()=>{
 it('captures raw JSON before the general JSON parser and verifies Svix headers',()=>{const app=readFileSync('src/server/app.ts','utf8');expect(app.indexOf("app.post('/api/webhooks/agentmail'")).toBeLessThan(app.indexOf("app.use(express.json"));expect(app).toContain('verifyAgentMailWebhook');});
